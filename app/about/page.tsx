  ...
  return (
    <>
      ...
      <section aria-labelledby="partis-titre" className="bowly-wide py-20 md:py-28">
        <Reveal>
          <p className="kicker text-crisp">Ce qui est décidé</p>
          <h2 id="partis-titre" className="poster-title text-bone mt-5">
            Quatre partis pris.
          </h2>
        </Reveal>
        ...
        <ul className="mt-14 grid gap-px md:grid-cols-2">
          {PARTIS_PRIS.map((parti, i) => (
            <Reveal as="li" key={parti.numero} delay={i * 0.05}>
              <div className="border-line h-full border-t py-9 md:pr-10">
                <span className="font-poster text-brand text-4xl leading-none" aria-hidden="true">
                  {parti.numero}
                </span>
                <h3 className="text-bone mt-5 text-2xl font-bold tracking-tight">
                  {parti.titre}
                </h3>
      ...
      <section
        ...
        className="border-line bg-void-2/60 border-y py-20 md:py-28"
      >
        <div className="bowly-wide">
          <Reveal>
            <p className="kicker text-plasma">Transparence</p>
            <h2 id="manque-titre" className="poster-title text-bone mt-5">
              Ce qui n&apos;existe
              <br />
              <span className="text-brand">pas encore.</span>
            </h2>
          ...
          <dl className="mt-12 grid gap-x-12 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
            {PAS_ENCORE.map(([k, v], i) => (
              <Reveal key={k} delay={i * 0.04}>
                ...
                <dd className="text-bone-dim mt-2.5 text-sm leading-relaxed">{v}</dd>
              </Reveal>
            ))}
          </dl>

          <Reveal delay={0.2}>
            <p className="text-bone-faint mt-12 max-w-2xl text-xs leading-relaxed">
              Les visuels de plats sont des illustrations générées pour ce site
              (voir <code className="text-bone-dim">scripts/generate-assets.mjs</code>).
              Ils ne représentent aucun plat réellement servi, et aucune photo
              n&apos;est empruntée à une autre enseigne.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bowly-wide py-20 md:py-28">
        <Reveal>
          <h2 className="poster-title text-bone">
            Envie d&apos;en être
            <br />
            <span className="text-brand">au début&nbsp;?</span>
          </h2>
